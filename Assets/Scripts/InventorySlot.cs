...
{
    // ѕосле последних изменений тут в принципе можно большу часть кода убрать 
    // надо будет этим зан€тьс€
    public void OnDrop(PointerEventData eventData)
    {
        if (transform.childCount == 0)
        {
            ...
            //if (transform.parent != null && transform.parent.tag == "LevelPage")
            //{
            //    BindingControl bc = transform.parent.parent.parent.GetComponent<BindingControl>();
            //    bc.SetCellSizeBasedOnTextSize(dropped, GetComponent<GridLayoutGroup>());
            //}
        }
    }
}
